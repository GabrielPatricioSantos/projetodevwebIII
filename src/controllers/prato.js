...
const add = async (req, res) => {
    try {
        ...
        let hasDuplicate = checkDuplicates(recipe)

        if(hasDuplicate)
            error = 'Ingredientes inválidos!'

        if(req?.file?.filename)
            prato['ID_Img'] = await imageController.add("Pratos", req.file.filename)
...
const get = async (req, res) => {
    try {
        ...
        if(prato == null)
            return res.redirect('http://localhost:5000/pratos')
        ...
        if(typeof prato?.ID_Img != 'undefined')
            Imagem = await imageController.get(prato.ID_Img)
